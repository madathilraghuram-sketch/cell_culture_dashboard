...
# Resolved from this script's location, not the working directory, so it also works on Streamlit Cloud.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_PATH = os.path.join(BASE_DIR, "data", "HER3_KO_HEK293F_12Week_Data.xlsx")
...
def main() -> None:
    ...
    if not os.path.isfile(DATA_PATH):
        st.error(
            f"Data file not found. Searched for:\n\n`{DATA_PATH}`\n\n"
            "Make sure `HER3_KO_HEK293F_12Week_Data.xlsx` is committed to the repository at "
            "`data/HER3_KO_HEK293F_12Week_Data.xlsx`, next to `dashboard.py` (the file name is case-sensitive "
            "on Streamlit Cloud)."
        )
        st.stop()
    try:
